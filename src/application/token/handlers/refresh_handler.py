...
class RefreshHandler:
    ...
    def __call__(self, command: RefreshCommand) -> TokenResult:
        ...
        try:
            ...
            old_token = uow.repositories.refresh_tokens.get_by_id(token_id)
            if old_token is None:
                raise NotFoundError("Refresh token не найден.")
            old_token.ensure_usable(now=now)

            account = uow.repositories.accounts.get_by_id(old_token.account_id)
            if account is None:
                raise NotFoundError("Аккаунт не найден.")
            if account.status != AccountStatus.ACTIVE:
                raise AccessDeniedError("Аккаунт недоступен.")

            session = uow.repositories.sessions.get_by_id(old_token.session_id)
            if session is None:
            ...
            uow.repositories.refresh_tokens.save(old_token)
            uow.repositories.refresh_tokens.add(new_token)
